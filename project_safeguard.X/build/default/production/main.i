...
void cont_write(timer * ,int * );


void _relay(int ,unsigned int );

# 94
void display1(unsigned int volt, unsigned int curr);

# 98
void wifi();

# 102
void display3(time *,unsigned int ,unsigned int);
...
__bit esp8266_isStarted(void);
__bit esp8266_restart(void);
...
__bit esp8266_start(unsigned char protocol, char* ip, unsigned char port);
...
__bit I2C_write(unsigned char data);
...
unsigned int ADC_read(unsigned char channel,bool type);
...
timer_index cts_index[5];
int no_cts;


unsigned int volt;
unsigned int current[3];
unsigned int volt_avg;
unsigned int current_avg;
...
volt_avg = 0;
current_avg = 0;
unsigned int volt_s = 0;
unsigned int curr_s = 0;
do { LCD_Write( (0x01 & 0xF0) >> 4 ); LCD_Write( 0x01 & 0x0F); } while ( 0 );
InitApp();
RTC_read(&time_now);
if((int)eeprom_read(0)==255){
StartService();
}
while(1){
...
for(n=0;n<4;n++){
current[n] = ADC_read(n,1);
curr_s += current[n];
}
if(counter == 50){
volt_avg = volt_s/50;
current_avg = curr_s/50;
volt_s = 0;
curr_s= 0;
counter = 0;
}

while((volt > over_volt) || PORTBbits.RB3){
_relay(5,0);
_breaker();
}
...
display3(&time_now,volt_avg,current_avg);


int t_index = (time_now.hour * 100)+(time_now.min * 1);
n = 0;
for(n=0;n<=no_cts;n++){
if((t_index >= cts_index[n].start_index) && (t_index <= cts_index[n].stop_index))
_relay(cts_index[n]._port,0);
else
_relay(cts_index[n]._port,1);
}
